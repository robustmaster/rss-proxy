...
3. **嗅探源**：输入想要抓取的网站域名（如 `https://example.com`），点击“查找订阅源”。
4. **获取链接**：在结果列表中点击你想要的源，系统会自动生成带 Token 的代理链接并复制到剪贴板。
5. **添加到阅读器**：将复制好的链接直接粘贴到 NetNewsWire, Reeder 或 Tiny Tiny RSS 等工具中即可。
